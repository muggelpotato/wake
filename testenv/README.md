...
- **`python testenv/drills_base.py`** the base module's own commands: the switches and where they land, the boats `/wake killemptyboats` takes and the one it leaves alone (a mob aboard), the order a reload answers in (either backend)
- **`python testenv/drills_module.py`** the module lifecycle: config toggles, repeated off/on cycles, the hot listener a feature claims from core, an enable that throws, seeding, the state prefix an export sweeps, the service seam (either backend)
